...
var GeektAddress = '0xd06a1935230c5bae8c7ecf75fbf4f17a04564ed8';    // mainnet address!

var web3 = new Web3(new Web3.providers.HttpProvider("http://localhost:8545"));
var GeektContract = web3.eth.contract(GeektABI).at(GeektAddress);
...
class App extends Component {
  ...
  signGuestbook() { // handles the "sign guestbook now" button functionality
    ...
    console.log(outerThis.state.defaultHandle,web3.fromAscii(outerThis.state.defaultCity),web3.fromAscii(outerThis.state.defaultState),web3.fromAscii(outerThis.state.defaultCountry));
    try {
      GeektContract.registerNewUser.estimateGas(outerThis.state.defaultHandle,web3.fromAscii(outerThis.state.defaultCity),web3.fromAscii(outerThis.state.defaultState),web3.fromAscii(outerThis.state.defaultCountry),{from:defaultAccount}, function(err, result){
        if(err) {
          throw err;
        } else {
          console.log("registerNewUser gas estimate : " + result);
          var myGasNum = result;
          GeektContract.registerNewUser.sendTransaction(outerThis.state.defaultHandle,web3.fromAscii(outerThis.state.defaultCity),web3.fromAscii(outerThis.state.defaultState),web3.fromAscii(outerThis.state.defaultCountry),{from:defaultAccount, gas: myGasNum}, function(err, result){
            if(err) {
              throw err;
            } else {
